...
```{r}
markovchaingame<-function(nn, winprob=0.48, start=100){
  output<-rep(NA, nn)
  output[1]<- start
  for (iteration in 2:nn){
    output[iteration]<-output[iteration-1] + 2*rbinom(100, 100, 0.48+0.01*)
  }
}
...
totalgames[1]<-100
for (turns in 2:100){
  totalhands[turns]<-rbinom(100, 100, 0.48+0.01*totalgames[turns-1])
}

...
library(rvest)
library(httr)
url<-GET("https://money.cnn.com/data/us_markets/")
page<-content(url, 'text')
cat(page)
...
link<-session %>%
  html_node("[wsod_symbol]") %>%
  html_attr("href")

link<-paste('https://money.cnn.com',link, sep='')
link<-paste(link, sep='')

moneytable<-data.frame(df2, link)
library(knitr)
...
```{r}
page_text<-read_html("https://money.cnn.com/data/us_markets/")
latestnews<-html_node(page_text, "#section_latestnews ~ ul")
latestnews

...
GR1=function(bet,bankroll,p)
{
  ...
  while(bet>0) {
    ...
    k=k+1
    if(k ==100){break} #breaks at 100 hands or when insufficient bankroll
...
```{r}
set.seed(1) #same code as before, just changing to 500
...
GR=function(bet,bankroll,p)
{
  ...
  while(bet>0) {
    ...
    {
      bankroll = bet + bankroll
      bet = (bet + bankroll)*0.1 #probability increases when the roll is less than probability
    }else{
      bankroll = bankroll-bet
      bet = bet
      }
...
markov_func=function(bet,bankroll,p)
{
  ...
  while(bet>0) {
    ...
    {
      bankroll <- bankroll +bet
      p = p+0.01 #probability increases when game is won
    }else{
      bankroll <- bankroll-bet
      p = prob #probability reset when lost
    }
    hand=hand+1
...
```{r}
  boostrapfun<-function(data, N, FUN=mean){
      #n variable takes the length of the data
    n <-length(data)
    #creating a matrix with columns equaling length of the data and N with the rows present in the data
  data_sample<-matrix(NA, nrow=N, ncol=n)
  #for loop populates the matrix with a sampling from your data
  for (i in 1:N){
   data_sample[i, ]<-sample(data, n, replicate = TRUE)
  }
  #returns an application of the function to the dataset

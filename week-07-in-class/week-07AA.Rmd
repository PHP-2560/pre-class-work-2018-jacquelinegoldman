...
for(i in 1:100){
  ...
  while(cash>0){ #when you have money, you get a result from the random uniform
    ...
    hand<-hand+1 
    #this is counting the hands
    if(hand==50000){ #if you had 100 hands, break
      cashvec[i]<-cash
      break
    }
    #breaks when insufficient cash to make bet
...
hand= 100
for (i in 1:hand){
  nextturn<- gamble_sim(1000, 100, 0.5)
...
iter<-0
for(i in 1:hands){
  outcomevec<-gamble_sim(1000, 100, 18/38)
  iter<-c(iter, outcomevec)
}
...
value<-markov_func(1000, 100, .48)
replicate(10, markov_func(1000, 100, .48))

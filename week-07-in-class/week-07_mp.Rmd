...
iter2<-0
for(i in 1:hands){
  outcomevec<-gamble_sim(1000, 100, 0.47368421052)
  iter2<-c(iter, outcomevec)
}

trials<-length(iter2) #gives the length of the iteration
success<-length(which(iter2<100)) #success is the length
probability<-success/trials #creates probability
print(probability)

trials<-length(iter2)
success<-length(which(iter2<500)) #same as part A but changed the length to where the number of iterations is less than 500
probability<-success/trials
print(probability)
...
```
    c. the mean time you go bust, given that you go bust within the first 5000 hands.
```{r}
timetobust<-iter[which(iter2<=5000)]
print(mean(timetobust))
...
```{r}
rep(100000, markov_func(1000, 100, 0.48))
```
